...
router.post('/register', (req, res) => {
  ...
  const { fname, lname, username, password, avatar, permission } = req.body;

  pool.getConnection(function(err, connection) {
    if (err) throw err; // not connected!

    // Use the connection
    connection.query(`INSERT INTO users (fname, lname, username, password, avatar, permission) VALUES ("${fname}", "${lname}", "${username}", "${password}", "${avatar}", ${permission})`, function (error, results) {
